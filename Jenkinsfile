pipeline {
    ...
    stages {
        ...
        stage('Docker Build') {
            steps {
                echo "==> [Stage 5] Building Docker image ${IMAGE_NAME}:${IMAGE_TAG}"
                sh """
                    docker build \\
        ...
        stage('Deploy') {
            steps {
                echo '==> [Stage 6] Deploying full stack via docker-compose'
                sh """
                ...
                sh '''
                    ...
                    echo "Backend did not respond in 3 minutes. Failing."
